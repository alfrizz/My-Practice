...
# ):
#     """
#     Forward-only micro-snapshot collector.

#     Purpose and guarantees
#     - Run a single, safe forward-only diagnostic snapshot and stash it at model._micro_snapshot.
#     - Non-invasive: no backward, no optimizer, no scheduler side-effects.
#     - Additive: only adds guarded probes and keys; does not remove or rename existing keys.
#     - Writes compact batch info into the canonical log via _append_log when possible.

#     Optional flags controlled by params.hparams
#     - MICRO_SAMPLE_K (int, default 0): sample per-segment latencies (p50/p90). Disabled by default.
#     - MICRO_EXTRA (bool, default False): compute param_bytes (total parameter bytes). Disabled by default.

#     Output
#     - snapshot dict saved at model._micro_snapshot containing these additive keys:
#       full_forward_ms, pred_extra_ms, preds_cpu_ms, num_segments, segments_per_sec, mean_seg_len,
#       gpu_peak_mb, gpu_reserved_mb, cpu_copy_bytes, device_syncs_count,
#       per_segment_p50_ms, per_segment_p90_ms, activation_mb,
#       out_shape, out_dtype, out_numel, collector_ms, dataloader_ms, param_bytes,
#       backward_ms, step_block_ms
#     """
#     from time import perf_counter
#     import inspect, torch, traceback, math

#     def _d(msg):
#         # Always try to write collector debug into canonical run log.
#         try:
...
#         except Exception:
#             pass
#         # Do not fallback to printing to notebook; keep output canonical only.

#     t_entry = perf_counter()

#     # --- Resolve missing args (globals then shallow caller frames) ---
#     try:
#         g = globals()
...
#     # Write canonical batch info into run log
#     try:
...
#                     batch_info = f"BATCH_INFO x0_shape={xshape}"
#                     if log_file is not None and callable(globals().get("_append_log")):
#                         _append_log(batch_info, log_file)
#             except Exception:
#                 pass
#     except Exception:
#         pass
...
#     try:
#         batch_line = f"BATCH_SHAPE B={B} groups={G} seq_len_full={seq_len_full} feat={feat_dim}"
#         if log_file is not None and callable(globals().get("_append_log")):
#             _append_log(batch_line, log_file)
#     except Exception:
#         pass
...
#     # param bytes optional (MICRO_EXTRA)
#     param_bytes = None
#     try:
#         micro_extra = False
#         try:
#             micro_extra = bool(params.hparams.get("MICRO_EXTRA")) if params is not None and hasattr(params, "hparams") else False
#         except Exception:
#             micro_extra = False
#         if micro_extra:
#             try:
...
#             except Exception:
#                 try:
#                     param_bytes = int(sum(int(p.numel()) for p in model.parameters()) * 4)
#                 except Exception:
#                     param_bytes = None
#     except Exception:
#         param_bytes = None
...
def collect_or_run_forward_micro_snapshot(
    ...
    - Default behavior preserves prior fast startup (MICRO_EXTRA False, MICRO_SAMPLE_K 0).
    """
    from time import perf_counter
...
#     # Use a visible tqdm alongside epoch tqdm by assigning distinct position and leave=True
#     try:
...
#             _append_log("    Extended snapshot fields available on model._micro_snapshot (inspect in Python):", log_file)
#             _append_log("      full_forward_ms, preds_cpu_ms, num_segments, segments_per_sec, mean_seg_len,", log_file)
#             _append_log("      gpu_peak_mb, gpu_reserved_mb, cpu_copy_bytes, device_syncs_count,", log_file)
#             _append_log("      per_segment_p50_ms, per_segment_p90_ms, activation_mb, out_shape, out_dtype, out_numel,", log_file)
#             _append_log("      collector_ms, dataloader_ms, param_bytes", log_file)
#             _append_log("", log_file)
...
#             # FIRST: produce/fetch a micro-snapshot (collector) with visible tqdm (if available)
#             try:
...
#                         # use position 1 so it does not overwrite the main epoch tqdm in position 0
#                         progress = _tqdm(total=1, desc="init_log micro-snapshot", position=1, leave=True)
#                 except Exception:
#                     progress = None
...
#             # Then emit detached snapshot info (shapes/losses/grads) if available
#             try:
...
#             # Emit richer MICRODETAIL from model._micro_snapshot (explicitly include every known key)
#             try:
#                 ms = getattr(model, "_micro_snapshot", None)
#                 if isinstance(ms, dict):
#                     parts = []
...
#                     # memory
#                     if ms.get("gpu_peak_mb") is not None:
#                         parts.append(f"gpuMB={ms['gpu_peak_mb']}")
#                     else:
#                         parts.append("gpuMB=None")
#                     if ms.get("gpu_reserved_mb") is not None:
#                         parts.append(f"gpuRes={ms['gpu_reserved_mb']}")
#                     else:
#                         parts.append("gpuRes=None")
#                     # cpu copy
...
def init_log(
    ...
    #     # Emit detached snapshot info (shapes/losses/grads) if available — guarded and robust
    #     try:
    #         if snapshot is not None:
    #             try:
    #                 raw_shape = snapshot.get("raw_reg_shape")
    ...
    #                         else:
    #                             # try to reduce tensor or handle non-finite
    ...
    #         # Emit richer MICRODETAIL from model._micro_snapshot (explicit and exhaustive)
    #         try:
    #             ms = getattr(model, "_micro_snapshot", None)
    #             if isinstance(ms, dict):
    #                 parts = []
    #                 # core timings and throughput (explicit None when missing)
    #                 parts.append(f"full_forward={ms.get('full_forward_ms', None):.2f}ms" if ms.get('full_forward_ms') is not None else "full_forward=None")
    #                 parts.append(f"preds_cpu={ms.get('preds_cpu_ms', None):.2f}ms" if ms.get('preds_cpu_ms') is not None else "preds_cpu=None")
    #                 parts.append(f"nseg={ms.get('num_segments', None)}")
    #                 parts.append(f"seg/s={int(ms.get('segments_per_sec')) if ms.get('segments_per_sec') is not None else 'None'}")
    ...
    #                 else:
    #                     parts.append("gpuRes=None")
    #                 # precise bytes (allocated/reserved)
    #                 if ms.get("gpu_allocated_bytes") is not None:
    #                     parts.append(f"gpuAllocBytes={ms['gpu_allocated_bytes']}")
    #                 else:
    #                     parts.append("gpuAllocBytes=None")
    #                 if ms.get("gpu_reserved_bytes") is not None:
    #                     parts.append(f"gpuResBytes={ms['gpu_reserved_bytes']}")
    #                 else:
    #                     parts.append("gpuResBytes=None")
    #                 # cpu copy
    #                 if ms.get("cpu_copy_bytes") is not None:
    #                     b = int(ms['cpu_copy_bytes'])
    #                     if b >= 1024 * 1024:
...
#                 la = snapshot.get("loss_aux")
#                 lm_f = float(lm) if lm is not None else float("nan")
#                 la_f = float(la) if la_f is not None else None
#                 waux = aux_w * (la_f if la_f is not None else 0.0)
#                 _append_log(f"DEBUG_LOSSES main={lm_f:.4e} aux={(la_f if la_f is not None else 'None')} w*aux={waux:.4e}", log_file)
...
#     # Optional AUX disabled flag (read-only; preserved from hparams when available)
#     aux_disabled = False
#     if hparams is not None:
#         aux_disabled = float(hparams.get("AUX_LOSS_WEIGHT", 0.0)) == 0.0
