...
#     3) Returns a dict mapping date -> (day_df, trades), even if trades == [].
#     """
...
#     print("Add_trade_signal_to_results …") # it calls "compute_continuous_signal" and "generate_trade_action" for each day
...
def run_trading_pipeline(
    ...
    pre_entry_decay: float,         # per‐minute decay rate applied to pre‐entry signal scores
    short_penal_decay: float,       # exponent penalty on very short trade durations
    ...
    smoothing_window: Optional[int] = None  # window size for optional centered rolling smoothing
) -> Optional[Dict]:
    """
