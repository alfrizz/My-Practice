...
class DualMemoryLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(n_feats, n_feats, kernel_size=3, padding=1)
...
def lstm_training_loop(
    ...
    *,
    ...
    """
    Train DualMemoryLSTM with a combined regression + binary‐classification loss.

    1) Move model to device; enable CuDNN autotune.
    2) Define mse_loss for continuous signal, bce_loss for binary head,
       and alpha_cls weight from params.hparams.
    3) Iterate epochs:
       a) Training pass in mixed precision:
          – reset hidden states per day/week
          – forward → pr, pc, _ = model(x_seq)
          – regression output lr = sigmoid(pr)
          – classification logits cls_logits = pc
          – loss = MSE(lr, sig_seq) + alpha_cls * BCEWithLogits(cls_logits, cls_seq)
          – backward, clip, optimizer.step, scaler.update
          – batch‐level cosine_sched.step(frac)
       b) Validation pass (no grad), compute metrics on both heads.
       c) Epoch‐level plateau_sched.step(val_rmse) after warmup.
       d) Early stopping and folder‐best checkpoint (_chp) logic.
    Returns best validation RMSE.
    """
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            for di in range(xb.size(0)):
                ...
                with autocast(device_type=device.type):
                    ...
                    loss_reg = mse_loss(lr, sig_seq)
                    loss_cls = bce_loss(cls_logits, cls_seq)
                    loss     = loss_reg + alpha_cls * loss_cls

                scaler.scale(loss).backward()

                # update metrics for regression
                train_rmse.update(lr,      sig_seq)
                train_mae .update(lr,      sig_seq)
                train_r2  .update(lr,      sig_seq)
        ...
        # f) Early stop & run‐best checkpoint
        if vl["rmse"] < best_val_rmse:
            best_val_rmse = vl["rmse"]
            best_state    = model.state_dict()
            best_tr       = tr.copy()
