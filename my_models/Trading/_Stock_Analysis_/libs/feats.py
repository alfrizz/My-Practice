...
#         hband = h.rolling(int(long_w), min_periods=1).max()
#         lband = l.rolling(int(long_w), min_periods=1).min()
...
#                 # safe division: treat zero or non-finite close_raw as NaN, then compute relative vwap
#                 close_safe = df_out["close_raw"].astype(float).replace(0, np.nan)
#                 s = ((s.astype(float) / close_safe) - 1.0).astype('float32')
#                 med = s.rolling(window=rz_window, min_periods=1).median()
#                 mad = (s - med).abs().rolling(window=rz_window, min_periods=1).median()
#             else:
...
# def extract_and_save_windows(loader, out_dir="trainings", features=None, h5_name="Xy.h5", chunk=1024, reserve=None):
#     """
#     Stream (xb,yb) from loader into HDF5, copy to disk memmaps, save metadata.
#     Minimal speedup: allocate/reserve space and grow in larger increments to avoid many small resizes.
...
#     batch0_n = xb0_np.shape[0]
#     L, F = int(xb0_np.shape[1]), int(xb0_np.shape[2])
#     dx, dy = xb0_np.dtype, yb0_np.dtype

#     # reserve: how many rows to allocate per growth step
#     if reserve is None:
#         reserve = max(chunk, batch0_n, 4096)

#     # create datasets with an initial allocation of `reserve`
#     X_alloc = reserve
#     y_alloc = reserve
#     X = f.create_dataset("X", shape=(X_alloc, L, F), maxshape=(None, L, F), dtype=dx, chunks=True)
#     y = f.create_dataset("y", shape=(y_alloc,), maxshape=(None,), dtype=dy, chunks=True)

#     # track used length separately from allocated length
#     X_used = 0
#     y_used = 0

#     def _ensure_space(ds, used, alloc, need):
#         """Return (alloc_new) after ensuring alloc >= used+need."""
#         if used + need <= alloc:
#             return alloc
#         # grow by multiples of reserve to reduce number of resizes
#         grow = max(reserve, need)
#         alloc_new = alloc
#         while used + need > alloc_new:
#             alloc_new += grow
#         ds.resize(alloc_new, axis=0)
#         return alloc_new

#     # append first batch
#     need = xb0_np.shape[0]
#     X_alloc = _ensure_space(X, X_used, X_alloc, need)
#     X[X_used:X_used + need] = xb0_np
#     X_used += need

#     need = yb0_np.shape[0]
#     y_alloc = _ensure_space(y, y_used, y_alloc, need)
#     y[y_used:y_used + need] = yb0_np
#     y_used += need

#     # stream remaining batches
#     for batch in tqdm(it, desc="Extracting windows", unit="batches"):
#         xb = batch[0].detach().cpu().numpy()
#         yb = batch[1].detach().cpu().numpy().reshape(-1)

#         need = xb.shape[0]
#         X_alloc = _ensure_space(X, X_used, X_alloc, need)
#         X[X_used:X_used + need] = xb
#         X_used += need

#         need = yb.shape[0]
#         y_alloc = _ensure_space(y, y_used, y_alloc, need)
#         y[y_used:y_used + need] = yb
#         y_used += need

#     # trim datasets to actual used length
#     if X_used != X.shape[0]:
#         X.resize(X_used, axis=0)
#     if y_used != y.shape[0]:
#         y.resize(y_used, axis=0)

#     # copy to memmaps (use chunk param to control memory)
...
#     N = sX[0]
#     for i in tqdm(range(0, N, chunk), desc="Copying HDF5→memmap", unit="rows"):
#         j = min(N, i + chunk)
#         Xm[i:j] = X[i:j]
#         ym[i:j] = y[i:j]
