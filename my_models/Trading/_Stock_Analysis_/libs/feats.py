...
#     def fast_psar(high: np.ndarray, low: np.ndarray, close: np.ndarray, step: float = 0.02, max_step: float = 0.2) -> np.ndarray:
#         """
#         Highly optimized Parabolic SAR using raw NumPy arrays.
#         Runs in < 0.5 seconds for 2M rows (vs 20+ minutes for ta library).
#         """
#         n = len(close)
...
#                 # Rule: PSAR cannot be above prior two periods' lows
#                 if i >= 2:
#                     psar[i] = min(psar[i], low[i-1], low[i-2])
#                 else:
#                     psar[i] = min(psar[i], low[i-1])
#                 if low[i] < psar[i]: # Flips to Bear
#                     bull = False
#                     psar[i] = ep
#                     ep = low[i]
#                     af = step
...
#             else:
#                 psar[i] = prev_psar + af * (ep - prev_psar)
#                 # Rule: PSAR cannot be below prior two periods' highs
#                 if i >= 2:
#                     psar[i] = max(psar[i], high[i-1], high[i-2])
#                 else:
#                     psar[i] = max(psar[i], high[i-1])
#                 if high[i] > psar[i]: # Flips to Bull
#                     bull = True
#                     psar[i] = ep
#                     ep = high[i]
#                     af = step     
...
#     # 1. Returns & Geometry
#     pbar.set_description("Indicators (Returns & Geometry)")
...
#     # 2. Moving Averages
#     pbar.set_description("Indicators (Moving Averages)")
#     for w in sma_ws:
...
#     pbar.set_description("Indicators (Momentum)")
#     for w in rsi_ws:
...
#     pbar.set_description("Indicators (Volatility)")
#     for w in atr_ws:
...
#     pbar.set_description("Indicators (Volume & Channels)")
...
#     pbar.set_description("Indicators (Specialized)")
...
#         h_arr, l_arr, c_arr = h.to_numpy(), l.to_numpy(), c.to_numpy()
        
#         new["psar"] = fast_psar(h_arr, l_arr, c_arr, step=step, max_step=max_step)
