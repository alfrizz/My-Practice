...
def simulate_trading(
    ...
    for ts, row in df.iterrows():
        ...
        # BUY branch
        if shares_max > 0 and row["action"] == 1: 
            action = "Buy" 
            shares_qty = min(shares_max, max(0, math.floor(shares_max * (row["buy_weight"]))))
            position += shares_qty
        ...
        if position > 0 and row["action"] == -1:
            action = "Sell"
            shares_qty = min(position, max(0, math.floor(position * (row["sell_weight"]))))
            position -= shares_qty
