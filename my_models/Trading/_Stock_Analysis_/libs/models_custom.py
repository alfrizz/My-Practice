...
def eval_on_loader(loader, model: nn.Module, clamp_preds: bool = True) -> tuple[dict, np.ndarray, np.ndarray, np.ndarray]:
    """
    Evaluate `model` on `loader` and return metrics plus CPU numpy predictions, baseline preds, and targets.

    Behavior and guarantees
    - Runs model in eval mode with torch.no_grad and sets model.h_short/model.h_long = None.
    - Accepts model outputs as either:
        * legacy single Tensor raw_reg (shape (W, T, 1)), or
        * new tuple/list (raw_reg, final, delta, base) where:
            - raw_reg is the original primary output (W, T, 1)
            - final is the model's final prediction (W, 1) for the last step
            - delta is predicted residual (W, 1)
            - base is causal baseline (W, 1)
      The function detects and uses the final/base elements when present.
    - Aggregates per-segment final-step predictions and targets, converts them to CPU numpy arrays.
    - Stores last_val_preds, last_val_targs, last_val_base on the model as torch.FloatTensors.
    - Returns a tuple: (metrics_dict, preds_np, base_preds_np, targets_np).
    """
    device = next(model.parameters()).device
    model.to(device).eval()
    model.h_short = model.h_long = None
    ...
    *,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for x_batch, y_signal, y_bin, y_ret, y_ter, rc, wd, ts_list, seq_lengths in \
            ...
            # ######################################################################################################
            # # Robust debug: handles seq_lengths as torch.Tensor or Python list
            # try:
            #     seq_list = seq_lengths.tolist()
            # except Exception:
            #     # seq_lengths may already be a list or other iterable
            #     try:
            #         seq_list = list(seq_lengths)
            #     except Exception:
            #         seq_list = ["<unrepr>"]
            
            # print("DBG_BATCH: seq_lengths batch:", seq_list)
            # if windows_tensor is None:
            #     print("DBG_BATCH: windows_tensor is None -> continue")
            # else:
            #     print("DBG_BATCH: windows_tensor.shape", tuple(windows_tensor.shape))
            #     print("DBG_BATCH: targets_tensor.shape", tuple(targets_tensor.shape))
            #     # show up to first 16 target values if available
            #     try:
            #         print("DBG_BATCH: targets[:16]", targets_tensor[:16].cpu().tolist())
            #     except Exception:
            #         print("DBG_BATCH: cannot show targets slice")
            #     # compute expected windows count from seq_lengths > 0 in a robust way
            #     try:
            #         expected = int(sum(1 for L in seq_list if int(L) > 0))
            #     except Exception:
            #         expected = "<error>"
            #     print("DBG_BATCH: expected_windows_from_seq_lengths", expected)
            # ######################################################################################################

            with _torch.amp.autocast(device_type="cuda", enabled=torch.cuda.is_available()):
                final_tensor, delta_tensor, base_tensor = model(windows_tensor) # each shaped (B,1)

                # ######################################################################################################
                # # Debug: after model(windows_tensor)
                # print("DBG_FWD: final_tensor.shape", tuple(final_tensor.shape) if 'final_tensor' in locals() else None)
                # print("DBG_FWD: delta_tensor.shape", tuple(delta_tensor.shape) if 'delta_tensor' in locals() else None)
                # print("DBG_FWD: base_tensor.shape", tuple(base_tensor.shape) if 'base_tensor' in locals() else None)
                # # count outputs
                # outs = final_tensor.reshape(final_tensor.size(0), -1)[:, 0] if 'final_tensor' in locals() else None
                # print("DBG_FWD: outputs_count", outs.shape[0] if outs is not None else None)
                # # raw compare
                # if outs is not None:
                #     print("DBG_COMPARE: outs_len, targets_len:", outs.shape[0], targets_tensor.shape[0])
                #     if outs.shape[0] < 32 and targets_tensor.shape[0] < 32:
                #         print("DBG_COMPARE: outs[:32]", outs.detach().cpu().tolist())
                #         print("DBG_COMPARE: targets[:32]", targets_tensor.detach().cpu().tolist())
                # ######################################################################################################
        ...
        avg_final_loss = epoch_final_loss_sum / max(1, epoch_loss_count)
        avg_final_rmse  = float(avg_final_loss ** 0.5)
        ...
        print(
            ...
            f"VALID→ RMSE={vl_rmse:.5f}, R²={vl_r2:.3f} |  "
            f"lr={current_lr:.2e} |  "
            f"loss={avg_loss:.5e} | final_rmse={avg_final_rmse:.5f} |  "
            f"improved={bool(improved)}"
        )
