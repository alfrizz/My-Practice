...
#     Sizing notes
#     - short_units and long_units are the total bidirectional output widths.
#       per-direction hidden_size = short_units // 2 (asserted even).
#     - layer_projection(in_dim, out_dim) returns nn.Identity when dims match, else nn.Linear.
#       Use it to adapt data between adjacent modules while keeping each module's hyperparameter authoritative.
#     """

#     def layer_projection(self, in_dim: int, out_dim: int) -> nn.Module:
#         """Return Identity if dims match else a tiny Linear adapter."""
...
#         # sanity for bidirectional LSTMs: total width must be even
#         assert short_units % 2 == 0, "short_units must be even (total bidirectional width)"
#         assert long_units % 2 == 0,  "long_units must be even (total bidirectional width)"
...
#             self.conv = nn.Conv1d(n_feats, CONV_CHANNELS,
#                                   kernel_size=conv_k,
...
#         # 1) TCN or Identity (TCN_CHANNELS authoritative for TCN outputs)
...
#         if self.use_delta:
#             _delta_lin = nn.Linear(flat_dim, 1)
#             nn.init.zeros_(_delta_lin.weight)
...
#         # 7) Delta baseline vs features predictions head
#         if self.use_delta:
#             delta = self.delta_head(norm_flat).squeeze(-1)
#         else:
...
# class ModelClass(nn.Module):
#     """
#     Compact stateful backbone used by the training loop.

#     Structure (keeps names and behavior from your last version)
#     - optional Conv1d -> optional TCN
#     - optional short bidirectional LSTM (short_lstm) producing `out_s`
#     - optional Transformer path (feature_proj -> pos_enc -> transformer) or Identity
#     - projection short2long -> optional long LSTM
#     - flatten/pool/last/attn head -> head_flat MLP that returns (B,1)
#     - optional delta head

#     Minimal modification (diagnostics-safe)
#     - Registers `self.input_proj` (nn.Linear) that maps raw feature vector length n_feats
#       -> upstream_dim. This parameter is present in every model variant and is intended
#       for external diagnostics that want per-raw-feature columns (log_epoch_feature_importance).
#       It is initialized identity-like when shapes allow, else Xavier.
#     - Keeps `self.feature_proj` semantics identical to the last version:
#       - when transformer active: adapts upstream_dim -> d_model
#       - when transformer inactive: a 2-D Linear upstream_dim -> upstream_dim (identity init)
#       This preserves forward shapes and behavior while ensuring diagnostics can read both
#       the canonical input projection and the runtime feature_proj.
#     """
#     def layer_projection(self, in_dim: int, out_dim: int) -> nn.Module:
#         """Return Identity if dims match else a tiny Linear adapter."""
#         return nn.Identity() if in_dim == out_dim else nn.Linear(in_dim, out_dim)
...
#         # 0) Conv1d + GroupNorm or Identity
#         CONV_CHANNELS = params.hparams["CONV_CHANNELS"]
#         if use_conv:
#             conv_k        = params.hparams["CONV_K"]
#             conv_dilation = params.hparams["CONV_DILATION"]
...
#         # canonical upstream dim for paths feeding Transformer / projection
#         upstream_dim = short_units if use_short_lstm else short_in

#         # --- Minimal addition: canonical input_proj (raw features -> upstream_dim)
#         # Registered purely for diagnostics (per-raw-feature mappings). Safe init.
#         self.input_proj = nn.Linear(n_feats, upstream_dim)
#         with torch.no_grad():
#             if n_feats == upstream_dim:
#                 try:
...
#             self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=layers)
#         else:
#             # keep a 2D projection weight available for diagnostics when transformer is disabled
#             self.feature_proj = nn.Linear(upstream_dim, upstream_dim)
#             with torch.no_grad():
#                 try:
#                     nn.init.eye_(self.feature_proj.weight)
#                 except Exception:
...
#         # 0) Conv + BN + ReLU
#         xc = x.transpose(1, 2) # (B, F, T)
#         xc = self.conv(xc); xc = self.bn(xc); xc = self.relu(xc)  # (B, C, T)
#         x  = xc.transpose(1, 2) #  (B, T, C)
...
#         # 3) Transformer — ensure canonical input_proj maps raw features to upstream before adapter
#         pre_tr = self.input_proj(out_s) if hasattr(self, "input_proj") else out_s
#         tr_in  = self.feature_proj(pre_tr)  # adapts upstream_dim -> d_model (or identity-like)
#         tr_in  = self.pos_enc(tr_in)
#         tr_out = self.transformer(tr_in.transpose(0, 1).contiguous())
#         out_t  = tr_out.transpose(0, 1).contiguous()
