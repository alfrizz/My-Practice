...
def apply_thresholds_per_day(
    ...
    *,
    ...
    thresh_mode: str,
    thresh_window: Optional[int] = None,
    thresh_mode_num: Optional[float] = None,
) -> pd.DataFrame:
    """
    Compute per-day or per-day rolling thresholds for col_signal and add:
      - 'signal_thresh' : per-row threshold (scalar per-day or rolling series)
      - 'gap_to_thresh'  : df[col_signal] - signal_thresh

    thresh_mode may be a string token or a numeric value.
    thresh_mode_num if provided overrides thresh_mode and forces numeric mode.
    thresh_window is used only for rolling modes.
    """
    # explicit numeric override
    if thresh_mode_num is not None:
        thresh_mode = float(thresh_mode_num)
    ...
    for day, day_df in tqdm(df.groupby(df.index.normalize()), desc="Thresh per day", leave=True):
        ...
        # scalar modes (including numeric)
        if isinstance(thresh_mode, (int, float)) or thresh_mode in {
            "median_nonzero", "mean_nonzero", "p90", "p95", "p99", "median", "mean"
        }:
            out["signal_thresh"] = day_thresh(series)
        else:
            # rolling modes: use provided window
            if thresh_mode == "roll_mean":
