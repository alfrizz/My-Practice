...
#######################################################################################################################################################   
    ...
    def step(self, actions):
        ...
        if self.terminal:
            print ('TRAIN STEP TERMINAL',self.model_name, 'over steps not terminal:', self.step_not_terminal)
            plt.clf()
            plt.plot(self.asset_memory,'r')
            plt.savefig(self.working_path+'/account_value_train_{}.png'.format(self.model_name))
            plt.close()
            df_total_value = pd.DataFrame(self.asset_memory)
            df_total_value.to_csv(self.working_path+'/account_value_train_{}.csv'.format(self.model_name))
            df_total_value.columns = ['account_value']
            df_total_value['daily_return']=df_total_value.pct_change(1)
            sharpe = (252**0.5) * df_total_value['daily_return'].mean() / df_total_value['daily_return'].std() 
            print("Training Sharpe Ratio: ", sharpe)
