...
class DualMemoryLSTM(nn.Module):
    """
    A two-stage, stateful LSTM for time-series that maintains both
    short-term (daily) and long-term (weekly) memories, with:
      • Variational Dropout on each LSTM’s outputs
      • LayerNorm on each LSTM’s hidden output for activation stability
      • Automatic hidden‐state resets at day/week boundaries
      • A time-distributed linear head producing one scalar per time-step
    """
    ...
    def __init__(
        ...
        Args:
          ...
          dropout_short– dropout rate after daily LSTM outputs
          dropout_long – dropout rate after weekly LSTM outputs

        Defines:
          short_lstm, long_lstm – nn.LSTM layers (batch_first=True)
          do_short, do_long     – nn.Dropout layers
          ln_short, ln_long     – nn.LayerNorm layers on hidden dims
          pred                  – nn.Linear mapping long_units → 1
          h_short, c_short      – hidden/cell states for daily LSTM
          h_long,  c_long       – hidden/cell states for weekly LSTM
        """
        super().__init__()
    ...
    def _init_states(self, batch_size: int, device: torch.device):
        """
        Allocate zero-tensors for hidden/cell states on given device.

        Shapes:
          h_short, c_short – (1, batch_size, short_units)
          h_long,  c_long  – (1, batch_size, long_units)
        """
        self.h_short = torch.zeros(1, batch_size, self.short_units, device=device)
        self.c_short = torch.zeros(1, batch_size, self.short_units, device=device)
        self.h_long  = torch.zeros(1, batch_size, self.long_units,  device=device)
        self.c_long  = torch.zeros(1, batch_size, self.long_units,  device=device)

    def reset_short(self):
        """
        Zero out daily LSTM’s hidden & cell states at each new day.
        """
    ...
    def reset_long(self):
        """
        Zero out weekly LSTM’s hidden & cell states at each new week,
        preserving the current short-term memory across that reset.
        """
        if self.h_long is not None:
            B, dev = self.h_long.size(1), self.h_long.device
            # stash short-term
            h_s, c_s = self.h_short, self.c_short
            # re-init both
            self._init_states(B, dev)
            # restore short-term on correct device
            self.h_short, self.c_short = h_s.to(dev), c_s.to(dev)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through dual-memory LSTM.

        Input:
          x – Tensor of shape (B, S, F) or (1, S, F), where F==n_feats.

        Output:
          Tensor of shape (B, S, 1) with per-step predictions.
        """
        # Handle extra leading dims: collapse to (B, S, F)
        if x.dim() > 3:
            *lead, S, F = x.shape
            x = x.view(-1, S, F)

        # Ensure feature dim is last
        ...
        # (Re)initialize h/c states if first forward or batch size changed
        if self.h_short is None or self.h_short.size(1) != B:
            self._init_states(B, dev)

        # Flatten parameters for cuDNN speed
        self.short_lstm.flatten_parameters()
        self.long_lstm.flatten_parameters()

        # --- Daily LSTM pass ---
        # out_short: (B, S, short_units)
        out_short, (h_s, c_s) = self.short_lstm(x, (self.h_short, self.c_short))
        self.h_short, self.c_short = h_s.detach_(), c_s.detach_()

        # Apply dropout + layer norm on daily output
        out_short = self.do_short(out_short)    # variational dropout
        out_short = self.ln_short(out_short)    # normalize hidden activations

        # --- Weekly LSTM pass ---
        out_long, (h_l, c_l) = self.long_lstm(out_short, (self.h_long, self.c_long))
        self.h_long, self.c_long = h_l.detach_(), c_l.detach_()

        # Dropout + layer norm on weekly output
        out_long = self.do_long(out_long)
        out_long = self.ln_long(out_long)

        # --- Final prediction head ---
        # Time-distributed linear: yields (B, S, 1)
